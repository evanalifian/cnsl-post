          ...
          <div class="mt-auto mb-4 pt-3 border-top border-secondary border-opacity-10">
            <a href="/profile" class="<?= $activeProfile ? $activeNavItem : $navItem ?>">
              ...
              <div class="d-flex flex-column lh-sm">
                <?php if (isset($data["user"]->display_name) || isset($data["currentUser"]->display_name)): ?>
                  <span class="fs-6 fw-bold text-white d-block"><?= isset($data["currentUser"]) ? $data["currentUser"]->display_name : $data["user"]->display_name ?></span>
                <?php endif ?>
                <span class="text-secondary opacity-75" style="font-size: 0.75rem">@<?= isset($data["currentUser"]) ? $data["currentUser"]->username : $data["user"]->username ?></span>
              </div>
